...
namespace Fungus.EditorUtils
{
    /// <summary>
    /// 1.縮放
    /// Flowchart window的主要運行邏輯在於,通過改變視窗大小的方式完成縮放的效果,視窗放大(調整GUI.matrix),block因為固定尺寸所以就會相對縮小
    /// 
    /// 2.位移
    /// FlowChart.ScrollPos像是一個中心點,後方網線跟block會根據這個中心點去計算,以下為解說範例:
    /// 1.只要玩家移動視窗,會改動ScrollPos,之後繪製全部的Block都會以這個Scroll為參考點去進行位移,但呈現的效果彷彿是玩家的視窗在移動
    /// 2.或者選定單數或複數的block去拖曳該block,那系統會將選定的建為一個list,之後這個list的block會獨自調整他們的nodeRect,來達到位移的效果
    /// 
    /// 網線有固定的寬(20),每20格會生成一條線(直豎都有)
    /// </summary>
    public class FlowchartWindow : EventWindow
    {
        ...
        protected struct BlockGraphics//連線的block
        {
            internal Color tint;
        ...
        public FungusState FungusState { get { return fungusState; } }
        ...
        /// <summary>
        /// 選取所有flowchart底下的block,並賦值給blocks
        /// </summary>
        ...
        /// <summary>
        /// 偵測block 的Name與搜尋的searchString是否吻合
        /// </summary>
        ...
        /// <summary>
        /// 偵測block 的command與搜尋的searchString是否吻合
        /// </summary>
        /// <param name="block"></param>
        /// <returns></returns>
        ...
        protected virtual void HandleEarlyEvents(Event e)
        {
            switch (e.type)
            {
                case EventType.MouseDown:
                    ...
                    if (!searchRect.Contains(e.mousePosition) && !popupRect.Contains(e.mousePosition))//如果再一次點擊沒有在上次圈選的rect範圍裏面 則清空
                    {
                        CloseBlockPopup();
                    }
        ...
        protected virtual void OnGUI()
        {
            ...
            InitStyles();//設置editor造型

            DeleteBlocks();

            UpdateFilteredBlocks();//更新篩選的block
            ...
            DrawFlowchartView(Event.current);//中間的主畫面

            // Draw selection box//拖曳滑鼠的藍色選取框
            if (Event.current.type == EventType.Repaint)
            {
                if (startSelectionBoxPosition.x >= 0 && startSelectionBoxPosition.y >= 0)
                {
                    GUI.Box(selectionBox, "", GUI.skin.FindStyle("SelectionRect"));//繪畫選擇框的功能
                }
            }
          ...
          //  Debug.Log("滑鼠位置"+Event.current.mousePosition);
           // Debug.Log("視窗大小=>" + flowchart.ScrollPos);
        ...
        protected virtual void DrawOverlay(Event e)//描繪flow chart 四腳功能視窗
        {
            ...
            GUILayout.BeginHorizontal();
            {
                ...
                if (e.type == EventType.Repaint)//如果有重劃的狀況?原本選定的rect依然選定?
                {
        ...
        protected virtual void DrawVariablesBlock(Event e)//左下的值
        {
            // Variables group
            GUILayout.BeginHorizontal();
            {
        ...
        protected Block GetBlockAtPoint(Vector2 point)//圈選的block
        {
        ...
        protected override void OnMouseDown(Event eve)
        {
            var hitBlock = GetBlockAtPoint(eve.mousePosition);

            // Convert Ctrl+Left click to a right click on mac
            if (Application.platform == RuntimePlatform.OSXEditor)//其他系統的另外判斷
            {
                if (eve.button == MouseButton.Left && eve.control)//ctrl
                {
                    eve.button = MouseButton.Right;
                }
            }
            ...
            switch (eve.button)
            {
                case MouseButton.Left:
                    if (!eve.alt)
                    {
                        if (hitBlock != null)
                        {
                            ...
                            startDragPosition = eve.mousePosition / flowchart.Zoom - flowchart.ScrollPos;
                            Undo.RecordObject(flowchart, "Select");

                            if (GetAppendModifierDown())//偵測是否有按按鍵
                            {

                                //ctrl clicking blocks toggles between
                                if (mouseDownSelectionState.Contains(hitBlock))//以有一個block
                                {
                                    RemoveMouseDownSelectionState(hitBlock);
                                }
                                else
                                {
                                    AddMouseDownSelectionState(hitBlock);//新增進複數點擊的群組                                                                                                                                                                                                                                                                                                               
                                }
                            }
                            else
                            {
                        ...
                        {
                            startSelectionBoxPosition = eve.mousePosition;
                ...
                case MouseButton.Right:
                    rightClickDown = eve.mousePosition;
                    eve.Use();
                    break;
            }
        }
        ...
        protected virtual void DrawFlowchartView(Event e)//中間的主畫面
        {
            // Calc rect for script view
            Rect scriptViewRect = CalcFlowchartWindowViewRect();
         //   Debug.Log("計算後的大小"+scriptViewRect);

            EditorZoomArea.Begin(flowchart.Zoom, scriptViewRect);//攝影機的角度
        ...
        protected virtual void DoZoom(float delta, Vector2 center)//縮放調整
        {
        ...
        protected virtual void DrawGrid()//後面的格子地板
        {
        ...
        protected virtual void DrawConnections(Block block)//連線所有相關的block
        {
            if (block == null)
            {
                return;
            ...
            foreach (var command in commandList)
            {
                ...
                foreach (var blockB in connectedBlocksWorkSpace)
                {
                    ...
                    Rect endRect = new Rect(blockB._NodeRect);//target block
                    endRect.x += flowchart.ScrollPos.x;
                    endRect.y += flowchart.ScrollPos.y;

                    Rect boundRect = new Rect();//邊界大小
        ...
        protected virtual void DrawRectConnection(Rect rectA, Rect rectB, bool highlight)//畫貝茲
        {
        ...
        /// <summary>
        /// 設置block 在flowchart視窗裡表現的圖形樣式 ,如通往其他block為單向 則正方形,多向則為六邊形
        ///先返回連線的block陣列
        /// </summary>
        /// <param name="block"></param>
        /// <returns></returns>
        protected virtual BlockGraphics GetBlockGraphics(Block block)//賦予2D圖
        {
            ...
            else
            {
                ...
                if (blockGraphicsUniqueListWorkSpace.Count > 1)//block通往複數的選項 外觀將改變成六邊形,所以獲得的圖案也不同
                {
        ...
        private void DrawBlock(Block block, Rect scriptViewRect)//生成要獲取的block 格子
        {

            #region 根據不同的文字長度有不同的寬,並且需要一開始生成block就從這裡先定義好高寬
            float nodeWidthA = nodeStyle.CalcSize(new GUIContent(block.BlockName)).x + 10;

            Rect tempRect = block._NodeRect;
            //如果文字太常會導致太寬,所以預設 上限跟下限
            ...
            if (FungusEditorPreferences.useGridSnap)
           {
            ...
            // Draw blocks
            var graphics = GetBlockGraphics(block);//賦予2D圖
           ...
           // Debug.Log("這個是什麼1?"+flowchart.ScrollPos);
            ...
            if (scriptViewRect.Overlaps(windowRelativeRect))
            {
                ...
                Texture2D tmpNormBg = nodeStyle.normal.background;

                // Draw untinted highlight
                if (block.IsSelected && !block.IsControlSelected)//選擇的綠框
                {
                ...
                if (block.IsControlSelected && !block.IsSelected)//按下ctrl鍵的選取
                {
                ...
                switch (block.FilterState)//篩選時的透明度
                {
                ...
                // Draw Event Handler labels 事件的提示字
                if (block._EventHandler != null)
                {
                    string handlerLabel = "";
            ...
            DrawConnections(block);//畫貝茲曲線連線的位置
        }
    }
}