...
namespace Fungus
{
    ...
    public class MenuDialog : MonoBehaviour
    {
        ...
        public bool CanChoose { get { return canChoose; }set { canChoose = value; } }
        ...
        /// </summary>
        /// <returns></returns>
        private IEnumerator MenuButtonCellAnimation()
        {

            yield return new WaitForSeconds(0.2f);
            bool tweenOver = false;
            foreach (var btn in cachedButtons)
            {

                yield return btn.gameObject.eSetCanvasGroup(0,
                    () =>
                    {
                        tweenOver = true;
                    });
            }
            yield return new WaitUntil(() => tweenOver);
        }
        ...
        public virtual void Clear()
        {
            ...
            for (int i = 0; i < optionButtons.Length; i++)
            {
                ...
                if (button != null)
                {
                    ...
                    button.gameObject.SetCanvasGroup(1);
                }
            }
        ...
        public virtual bool AddOption(string text, bool interactable, bool hideOption, Block targetBlock,Action<int> cbInt=null,Action cb=null)
        {
            var block = targetBlock;
            cbInt(nextOptionIndex);
            ...
            UnityEngine.Events.UnityAction action = delegate
            {
                ...
                {
                    SwitchMenuButton();
                    yield return MenuButtonCellAnimation();

                    int option = nextOptionIndex;
                ...
                {
                    canChoose = false;
                    targetBlock.GetFlowchart().StartCoroutine(StartExecute());
                }
        ...
        private bool AddOption(string text, bool interactable, bool hideOption, UnityEngine.Events.UnityAction action)
        {
            ...
            var btnTxt = button.transform.Find("Root").Find("ButtonImage").Find("ButtonText").GetComponent<Text>();
            var selectTxt= button.transform.Find("Root").Find("SelectImage").Find("SelectText").GetComponent<Text>();
