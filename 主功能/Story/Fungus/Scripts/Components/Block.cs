...
namespace Fungus
{
    ...
    [AddComponentMenu("")]
    [Serializable]
    public class Block : Node,IComparable<Block>
    {
        ...
        public bool FirstPrioritySetting = false;
        ...
        public virtual IEnumerator Execute(int commandIndex = 0, Action onComplete = null)//執行這個block的所有command內容
        {
            ...
            if (suppressAllAutoSelections || SuppressNextAutoSelection) //block all select  
            {
            ...
            /*    if (commandList[i].CanNotSkipCommand() > 0)
                {
                    flowchart.mStoryControl.DontSkipStoryPlot();
                }
                else
                {
                    flowchart.mStoryControl.CanSkipStoryPlot();
                }*/
                ...
                try
                {
                    //   command.CanNotSkipCommand();
                    if (flowchart.mStoryControl.isSkipPlay) 
                    {
                        command.AdjustCommandExecuteSpeedAcceleration(true);
                    }

                    /*bool isProhibit = false;
                    foreach (var prohibitExecuteCommand in flowchart.StopSkipCommands) 
                    {
                        if (command.GetType().Name==prohibitExecuteCommand)
                        {
                            if (prohibitExecuteCommand== "ShowFirstEditName"&& MessageResponseData.Instance.ProtoResponse10301.sex < 2) 
                            {
                                break;
                            }
                            isProhibit = true;
                    ...
                    bool isProhibit = command.CanNotSkipCommand();
        ...
        {
            return blockName.CompareTo(a.blockName);
        }
