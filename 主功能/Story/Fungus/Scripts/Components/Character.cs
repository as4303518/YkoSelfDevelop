...
namespace Fungus
{
    ...
    public class Character : MonoBehaviour, ILocalizable, IComparer<Character>
    {
        ...
        [SerializeField] protected SkeletonGraphic mSkeletonGraphic=null;

        public SkeletonGraphic aSkeletonGraphic { 
            get {
            /*    if (mSkeletonGraphic!=null) {
                    mSkeletonGraphic.gameObject.transform.localScale = mSet.Scale;
                }*/

                return mSkeletonGraphic;
            } 
        }//skins 因為spine裡面已經有造型選擇 造理說只會有一個
        ...
        /// </summary>
        /// <returns></returns>
        public string GetCharaNumString()
        {
            string num = "";

            if (nameText.StartsWith("char")) {//預設前墜char
                
                num=nameText.Substring(nameText.IndexOf("char")+4);
            }
            else//角色明並非預設前墜
            {
                int tarNum = 0;
                ...
                {
                    var tarChar = nameText.ToCharArray()[i];

                    if (int.TryParse(tarChar.ToString(),out tarNum)) 
                    {
                        tarIndex = i;
                        break;
                    }
                }
                num =nameText.Substring(tarIndex);
            }
            ...
            {
                var verNum = num.ToString();
                for (int i=0;i<3-num.ToString().Length;i++) 
                {
                   verNum=verNum.Insert(0, "0");
                }
                num =verNum;
            }
