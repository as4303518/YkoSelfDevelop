...
namespace Fungus
{
    ...
    public class SpawnObject : Command
    {
        ...
        public override void OnEnter()
        {
            ...
            StartCoroutine(GetTranslateTextFromChild(newObject));
        ...
        /// 文字皆進行多語言翻譯
        /// </summary>
        private IEnumerator GetTranslateTextFromChild( GameObject obj)
        {

            Text[] childTexts = obj.GetComponentsInChildren<Text>();

            foreach (var tar in childTexts)
            {
                string str = tar.text;
                yield return TranslateOfCsv.eGetSpecifyValueOfCsvFile(str, ParentBlock.GetFlowchart().mLanguage, text => {
                    tar.text = text;
                });
            }
        }
