...
namespace Fungus.EditorUtils
{
    ...
    public class BlockEditor : Editor
    {
        ...
        public virtual void DrawBlockName(Flowchart flowchart)//開頭名字
        {
            serializedObject.Update();

            SerializedProperty blockNameProperty = serializedObject.FindProperty("blockName");

            if (!tar.FirstPrioritySetting)//設置剛生成的Block export priority
            {
                tar.FirstPrioritySetting = true;
            }
            ...
            if(EditorGUI.EndChangeCheck())//檢測從開始到現在的數值變化
            {
        ...
        public virtual void DrawBlockGUI(Flowchart flowchart)//中間的選項
        {
            serializedObject.Update();
            ...
            if (block == flowchart.SelectedBlock)
            {
                ...
                DrawEventHandlerGUI(flowchart);//開始的方法
        ...
        public virtual void DrawButtonToolbar()//最下方的新增cammand 命令列
        {
            ...
            // Add Button
            if (GUILayout.Button(addIcon))//新增command
            {
            ...
            // Duplicate Button
            if (GUILayout.Button(duplicateIcon))//複製command
            {
                Copy();
                Paste();
            }
            ...
            // Delete Button
            if (GUILayout.Button(deleteIcon))//刪除command
            {
                Delete();
            }
        ...
        protected virtual void DrawEventHandlerGUI(Flowchart flowchart)//block 開始執行的方法 
        {
        ...
        public static void BlockField(SerializedProperty property, GUIContent label, GUIContent nullLabel)//block的下拉選項
        {
            Flowchart flowchart = null;
            var block = property.objectReferenceValue as Block;
          /*  if (block == null)
            {
                if (LastSelectedGameObjectTracker.lastSelectedObject == null)
                {
                    flowchart = Flowchart.GetInstance();
                }
                else
                {
                    flowchart = LastSelectedGameObjectTracker.lastSelectedObject.GetComponent<Flowchart>();
                }
            }
            else
            {
                flowchart = block.GetFlowchart();
                Debug.Log("直接找block拿");
            }*/
            //顯示優先級
            //最後選擇物件>block上原本就有的block的flowchart>直接拿flowchart上的預設(容易拿錯,只拿在hierarchy上活躍的)
            if (LastSelectedGameObjectTracker.lastSelectedObject!=null) 
            {
                if (!LastSelectedGameObjectTracker.lastSelectedObject.TryGetComponent<Flowchart>(out flowchart)) 
                {
                    if (block == null)
                    {
                        flowchart = Flowchart.GetInstance();
    ...
    [CustomPropertyDrawer(typeof(BlockListAttribute))]
    public class BlockListAttributeEditor : PropertyDrawer
    {

        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            
            // BlockListAttribute tar=attribute as BlockListAttribute;
            GameObject targetObject = property.serializedObject.targetObject as GameObject;
            BlockEditor.BlockField(
                       property,
